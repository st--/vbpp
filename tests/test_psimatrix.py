...
def Psi(session_tf):
    ...
    kernel = gpflow.kernels.SquaredExponential(input_dim=Data.Z.shape[1],
        variance=Data.variance, lengthscales=Data.lengthscales)
    with gpflow.params_as_tensors_for(kernel, feature):
        Psi_tensor = tf_calc_Psi_matrix(kernel, feature, Data.domain)
    return session_tf.run(Psi_tensor)
